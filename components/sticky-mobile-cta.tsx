 ...
 *  - A slim floating pill (quote button + call/WhatsApp icons), padded with
 *    env(safe-area-inset-bottom) for iPhone home-indicator devices.
...
export default function StickyMobileCta() {
  return (
    ...
    >
      {/* Slim floating pill: one primary action, call + WhatsApp as icon buttons.
          ~52px tall instead of a 64px full-width slab, so more page shows. */}
      <div className="pointer-events-auto mx-auto flex max-w-md items-center gap-1.5 rounded-full border border-black/5 bg-white/95 p-1.5 shadow-[0_6px_24px_rgba(10,36,99,0.18)] backdrop-blur">
        <Link
          href="/get-quote"
          className="flex h-10 flex-1 items-center justify-center gap-2 rounded-full bg-[#EE5824] px-4 text-[14px] font-semibold text-white active:brightness-95"
        ...
        <a
          href={`tel:${PHONE}`}
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#0A2463] text-white active:brightness-95"
        ...
        <a
          ...
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#25D366] text-white active:brightness-95"
